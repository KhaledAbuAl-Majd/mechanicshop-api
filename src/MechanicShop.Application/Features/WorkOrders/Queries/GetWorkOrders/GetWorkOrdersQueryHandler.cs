    ...
    : IRequestHandler<GetWorkOrdersQuery, Result<PaginatedList<WorkOrderListItemDto>>>
{
    ...
    public async Task<Result<PaginatedList<WorkOrderListItemDto>>> Handle(GetWorkOrdersQuery query, CancellationToken ct)
    {
        ...
        workOrderQuery = ApplyFilters(workOrderQuery, query);

        if (!string.IsNullOrWhiteSpace(query.SearchTerm))
        {
            workOrderQuery = ApplySearchTerm(workOrderQuery, query.SearchTerm);
        }

        workOrderQuery = ApplySorting(workOrderQuery, query.SortColumn, query.SortDirection);

        var source = workOrderQuery.Select(wo => new WorkOrderListItemDto
        {
            WorkOrderId = wo.Id,
            ...
            Customer = wo.Vehicle != null && wo.Vehicle.Customer != null ? wo.Vehicle.Customer.Name : string.Empty,
            Labor = wo.Labor != null ? wo.Labor.FirstName + " " + wo.Labor.LastName : null,
            State = wo.State,
            RepairTasks = wo.RepairTasks.Select(rt => rt.Name).ToList()
        });

        return await source.ToPaginatedListAsync(query.Page, query.PageSize, ct);
    }
